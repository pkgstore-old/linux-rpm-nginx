...
%{__sed} -i -e 's#PROFILE=SYSTEM#HIGH:!aNULL:!MD5#' nginx.conf
%endif
...
find %{buildroot} -type f -empty -exec %{__rm} -f '{}' \;
find %{buildroot} -type f -iname '*.so' -exec %{__chmod} 0755 '{}' \;

%{__install} -p -D -m 0644 ./nginx.service \
  %{buildroot}%{_unitdir}/nginx.service
%{__install} -p -D -m 0644 %{SOURCE11} \
  %{buildroot}%{_sysconfdir}/logrotate.d/nginx

%{__install} -p -d -m 0755 %{buildroot}%{_sysconfdir}/systemd/system/nginx.service.d
%{__install} -p -d -m 0755 %{buildroot}%{_unitdir}/nginx.service.d
...
%{__install} -p -d -m 0700 %{buildroot}%{_localstatedir}/lib/nginx
%{__install} -p -d -m 0700 %{buildroot}%{_localstatedir}/lib/nginx/tmp
...
%{__install} -p -m 0644 %{SOURCE920} \
  %{buildroot}%{_sysconfdir}/nginx/vhosts.d/00-server.default.conf

%if 0%{?with_mailcap_mimetypes}
%{__rm} -f %{buildroot}%{_sysconfdir}/nginx/mime.types
%endif

%{__install} -p -D -m 0644 %{_builddir}/nginx-%{version}/objs/nginx.8 \
  %{buildroot}%{_mandir}/man8/nginx.8

%{__install} -p -D -m 0755 %{SOURCE13} %{buildroot}%{_bindir}/nginx-upgrade
%{__install} -p -D -m 0644 %{SOURCE14} %{buildroot}%{_mandir}/man8/nginx-upgrade.8

for i in ftdetect ftplugin indent syntax; do
  %{__install} -p -D -m644 contrib/vim/${i}/nginx.vim \
...
%{__install} -m 755 %{SOURCE921} \
  %{buildroot}%{_libexecdir}/nginx-ssl-pass-dialog
# SSL generator: nginx-ssl-gencerts.
%{__install} -m 755 %{SOURCE922} \
  %{buildroot}%{_libexecdir}/nginx-ssl-gencerts
# SSL generator: nginx-ssl-gencerts-dhparam.
%{__install} -m 755 %{SOURCE923} \
  %{buildroot}%{_libexecdir}/nginx-ssl-gencerts-dhparam
